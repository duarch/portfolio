...
export default function App() {
  ...
  useEffect(() => {
    ...
    const checkPieces = setInterval(() => {
      if (
        piecesRef.current.length === puzzlePositions.length &&
        heroRef.current
      ) {
    ...
    <div className="app">
        ...
        <div className="hero-content">
          ...
          <div className="hero-actions">
            <a
              ...
              href="#projects"
              aria-label={`${t("viewProjects")} - ir para secao de projetos`}
            >
              {t("viewProjects")}
            </a>
            <a
              className="btn btn-ghost"
              href="https://api.whatsapp.com/send?phone=5561920028650&text=👋"
              target="_blank"
              rel="noreferrer"
              aria-label={`${t("contactMe")} via WhatsApp - abre em nova aba`}
            >
              {t("contactMe")}
            </a>
            <a
              className="btn btn-ghost"
              href={getResumeUrl()}
              aria-label={`Ver meu ${t("curriculum").toLowerCase()} completo`}
            >
              {t("curriculum")}
            </a>
          </div>
          <div
            ...
            aria-live="polite"
            aria-label={`Dica: Role a pagina para ver a animacao do quebra-cabeca se montando`}
          >
        ...
        <div className="timeline" role="list">
          {t("timeline").map((item) => (
            <div
              ...
              role="listitem"
            >
              <div className="timeline-year" aria-label={`ano ${item.year}`}>
                {item.year}
              </div>
          ...
          {t("projects").map((project) => (
            ...
            >
              <div className="project-visual">
                <img
                  src={project.image}
                  alt={`Imagem do projeto ${project.name}: ${project.description}`}
                  loading="lazy"
                />
              </div>
              <div className="project-info">
                ...
                <a
                  ...
                  target="_blank"
                  rel="noreferrer"
                  aria-label={`${t("viewProject")} ${project.name} (abre em nova aba)`}
                >
        ...
        <div className="split">
          ...
          <div className="split-visual reveal" aria-hidden="true">
            <div
              ...
              aria-label="Visualizacao decorativa de pecas de quebra-cabeca em orbita"
            >
      ...
      >
        <div className="cta reveal">
          ...
          <div className="contact-buttons">
            <a
              className="btn btn-primary"
              href="mailto:andre@duar.ch"
              aria-label={`Enviar email para andre@duar.ch`}
            >
              📧 Email
            </a>
            <a
      ...
      <footer className="footer" role="contentinfo">
        ...
        <nav
          ...
          aria-label="Links adicionais e redes sociais"
        >
          <a
            href={getResumeUrl()}
            aria-label={`Ver meu ${t("curriculum").toLowerCase()} completo`}
          ...
          <a
            href="https://github.com/duarch"
            aria-label={`Perfil ${t("footerGithub")} de Andre`}
          >
            {t("footerGithub")}
          </a>
          <a
            href="https://twitter.com/andrebh"
            aria-label={`Perfil ${t("footerTwitter")} de Andre`}
          >
            {t("footerTwitter")}
          </a>
          <a
            href="https://dev.to/duarch"
            aria-label={`Perfil ${t("footerDev")} de Andre`}
          >
            {t("footerDev")}
          </a>
        </nav>
